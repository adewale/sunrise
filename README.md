...
Sunrise also runs scheduled refreshes every four hours, anchored at 06:00 in the deployment's configured cron timezone: 02:00, 06:00, 10:00, 14:00, 18:00, and 22:00. Cloudflare cron expressions are stored in `wrangler.jsonc`; adjust them in your fork if you want different local collection times. Scheduled refreshes use the latest stored owner session token, send cached GitHub ETags where available, compare the new GitHub snapshot with the previous one, and skip queue processing when nothing changed.
