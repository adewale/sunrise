...
- **Reachability fidelity.** The first discovery properties hardened `dedupeActionItems` and `orderActionItems`, but neither helper had a production caller. `discoverFromGitHub` still used an order-sensitive `dedupeChanges`, so the live bug survived behind green properties. Fix: export and test the production deduplicator itself. `test/scanner.test.ts` generates duplicate canonical keys, reordered sources, and malformed timestamps; restoring the old implementation fails after one case and shrinks to the problematic pair.
