...
function renderRuns(props: any) {
  const runs = props.runs ?? props;
...
function renderItemPage(props: any) {
  if (!props.item) return '<section class="section panel"><p class="eyebrow">Inbox card</p><h1>Not found</h1><div class="empty-state"><strong>Card not found.</strong><p>This item may have been resolved or ignored.</p></div><p><a class="button ghost" href="/dashboard">Back to inbox</a></p></section>';
  return `<section class="section panel"><div class="section-head"><div><p class="eyebrow">Inbox card</p><h1>GitHub loop</h1><p class="muted">Open this card in its own tab, share the Sunrise URL, or continue through to GitHub.</p></div><a class="button ghost" href="/dashboard">Back to inbox</a></div><div class="item-list">${renderItem(props.item, props.signedInAs)}</div></section>`;
}

function renderDashboard(props: any) {
  const summary = props.refreshSummary ? `<section class="refresh-summary ${props.refreshSummary.status === 'no_change' ? 'quiet' : ''}"><strong>${props.refreshSummary.status === 'no_change' ? 'No GitHub changes.' : 'Collected GitHub.'}</strong><span>${props.refreshSummary.status === 'no_change' ? 'The current snapshot matches the previous refresh.' : `${props.refreshSummary.candidateCount ?? 0} found · ${props.refreshSummary.resolvedCount ?? 0} resolved`}</span></section>` : '';
  return `${summary}${props.notice ? `<section class="setup-status ready">${escapeHtml(props.notice.message)}</section>` : ''}
  ...
  <div class="dashboard-layout"><section class="inbox panel"><div class="item-list inbox-list">${renderGroupedItems(props.items, props.signedInAs)}</div>${renderPagination(props.pagination)}</section><aside class="marginalia" aria-label="Dashboard statistics"><section class="panel stat-card unresolved-card"><p class="eyebrow">Unresolved on GitHub</p><div class="stat-list">${renderUnresolvedLinks(props.unresolvedLinks)}</div></section><section class="panel stat-card"><p class="eyebrow">Counts</p><div class="stat-list">${renderStat('PRs', props.counts.pullRequests)}${renderStat('Issues', props.counts.issues)}${renderStat('My PRs · own repos', props.counts.myPrsOwnRepos)}${renderStat('My PRs · elsewhere', props.counts.myPrsOtherRepos)}${renderStat('PRs to my repos', props.counts.prsInMyRepos)}</div></section></aside></div>`;
}

function renderGroupedItems(items: GitHubActionItem[], ownerLogin: string) {
  if (!items.length) return '<div class="empty-state"><strong>All clear.</strong><p>No unresolved GitHub loops are in your inbox right now.</p></div>';
  const groups = new Map<string, GitHubActionItem[]>();
  for (const item of items) {
    const label = timeSectionLabel(item.updatedAt);
    groups.set(label, [...(groups.get(label) ?? []), item]);
  }
  return ['Today', 'Yesterday', 'Earlier this week', 'Older', 'Earlier'].filter((label) => groups.has(label)).map((label) => `<section class="time-section"><h2>${label}</h2>${groups.get(label)!.map((item) => renderItem(item, ownerLogin)).join('')}</section>`).join('');
}
...
function renderChangelog(props: any) {
  return `<section class="section panel"><div class="section-head"><div><p class="eyebrow">What changed</p><h1>Changelog</h1><p class="muted">Privacy-preserving release notes for you and your coding agent. Sunrise does not register this deployment upstream.</p></div><a class="button ghost" href="/settings">Settings</a></div><div class="config-card"><p><span>Current version</span><code>${escapeHtml(props.version?.version ?? 'unknown')}</code></p><p><span>Upgrade contract</span><code>${escapeHtml(props.version?.upgradeContract ?? 'docs/agent-upgrade-contract.md')}</code></p></div><pre class="changelog-text">${escapeHtml(props.changelog ?? '')}</pre></section>`;
}

function renderSettings(settings: UserSettings, props: any = {}) {
  const options = [25, 50, 100].map((n) => `<option value="${n}"${settings.inboxPageSize === n ? ' selected' : ''}>${n} events</option>`).join('');
  const update = props.update ?? {};
  const version = props.version ?? SUNRISE_VERSION;
  return `<section class="section panel settings-panel"><div class="section-head"><div><p class="eyebrow">Preferences</p><h1>Settings</h1><p class="muted">Keep Sunrise quiet, skimmable, and tuned to how much GitHub you want at once.</p></div></div><form method="post" action="/settings" class="settings-form"><label><span>Inbox page size</span><select name="inboxPageSize">${options}</select></label><p class="muted">Pagination starts after this many events. Default is 50.</p><label class="checkbox-row"><input type="checkbox" name="includeSubscribedNotifications"${settings.includeSubscribedNotifications ? ' checked' : ''}><span>Include watched-repository notifications</span></label><p class="muted">Off by default. Mentions, assignments, review requests, authored threads, invitations, owned-repo PRs, workflow failures, and security alerts still appear.</p><button class="button primary">Save settings</button></form></section><section class="section panel"><div class="section-head"><div><p class="eyebrow">Sunrise version</p><h2>${update.hasUnseenChangelog ? 'What changed' : 'Up to date locally'}</h2><p class="muted">Current version ${escapeHtml(update.currentVersion ?? version.version)}. No deployment telemetry is sent upstream.</p></div><a class="button primary" href="/changelog">View changelog</a></div><div class="config-card"><p><span>Agent upgrade contract</span><code>${escapeHtml(version.upgradeContract)}</code></p><p><span>Upstream</span><code>${escapeHtml(version.upstream)}</code></p></div></section>`;
}
...
function renderUnresolvedLinks(rows: Array<{ label: string; count: number; href: string; query?: string }> = []) {
  if (!rows.length) return '<div class="empty-state compact"><strong>All clear.</strong><p>Nothing unresolved in the current GitHub snapshot.</p></div>';
  return rows.map((row) => `<a class="stat unresolved-link" href="${escapeHtml(row.href)}" target="_blank" rel="noreferrer" title="${escapeHtml(row.query ? `Opens GitHub: ${row.query}` : 'Opens GitHub')}"><span>${escapeHtml(row.label)}<em>Open in GitHub</em></span><strong>${row.count} ↗</strong></a>`).join('');
}
...
function renderDesignLanguage() {
  ...
  <section class="section panel"><div class="section-head"><div><p class="eyebrow">Action item</p><h2>Dashboard row</h2></div><span class="badge">example</span></div><div class="item-list">${renderItem(sampleItem)}</div></section>
...
function renderItem(item: GitHubActionItem, ownerLogin = '') {
  ...
  const chips = [itemTypeLabel(item), itemRelationshipLabel(item, ownerLogin)].filter(Boolean).map((chip) => `<span class="chip">${escapeHtml(chip)}</span>`).join('');
  const repoOwner = item.repo.split('/')[0] || 'github';
  const repoChip = item.repo ? `<a class="chip repo-chip" href="https://github.com/${escapeHtml(item.repo)}" target="_blank" rel="noreferrer">${escapeHtml(item.repo)} ↗</a>` : '';
  const author = item.evidence?.author;
  const authorAvatar = author ? `<img class="author-avatar" src="https://github.com/${escapeHtml(author)}.png?size=40" alt="" loading="lazy">` : '';
  const check = item.evidence?.checks ? `<span class="check-status ${escapeHtml(item.evidence.checks)}" title="Checks ${escapeHtml(item.evidence.checks)}" aria-label="Checks ${escapeHtml(item.evidence.checks)}"></span>` : '';
  return `<article class="item" id="item-${escapeHtml(item.id)}"><a class="item-time" href="/items/${encodeURIComponent(item.id)}" title="Open this card" aria-label="Open card updated ${escapeHtml(when.date)} ${escapeHtml(when.time)}"><time datetime="${escapeHtml(item.updatedAt)}"><span>${escapeHtml(when.date)}</span><strong>${escapeHtml(when.time)}</strong></time></a><div class="item-main"><div class="item-topline"><div class="item-signals"><span class="type-icon" aria-hidden="true">${escapeHtml(itemIcon(item))}</span><img class="repo-avatar" src="https://github.com/${escapeHtml(repoOwner)}.png?size=40" alt="" loading="lazy">${authorAvatar}${check}</div><div class="chips">${chips}${repoChip}</div></div><a class="item-title" href="${item.url}">${escapeHtml(item.title)}</a><p>${escapeHtml(item.reason)}</p><p class="action">${escapeHtml(item.suggestedAction)} <span class="relative-time">· updated ${escapeHtml(relativeTime(item.updatedAt))}</span></p></div></article>`;
}
...
function itemTypeLabel(item: GitHubActionItem) {
  ...
  if (item.kind.includes('discussion')) return 'Discussion';
  return item.kind.replaceAll('_', ' ');
}

function itemIcon(item: GitHubActionItem) {
  if (isPullRequestItem(item)) return '⑂';
  if (isIssueItem(item)) return '○';
  if (item.kind.includes('discussion')) return '◌';
  if (item.kind === 'workflow_failure') return '×';
  return '•';
}
...
function itemRelationshipLabel(item: GitHubActionItem, ownerLogin = '') {
  ...
  if (item.kind === 'assigned') return 'Assigned to you';
  if (item.kind === 'notification' && item.evidence?.notificationReason === 'subscribed') return 'Watched repo';
...
.metrics{display:grid;grid-template-columns:repeat(12,minmax(0,1fr));gap:12px;margin:0}.metric{grid-column:span 3;min-width:0;padding:18px;border-radius:10px}.metric span{display:block;color:var(--muted);font-size:13px;font-weight:600}.metric strong{font-family:var(--font-display);font-variant-numeric:tabular-nums;font-size:38px;font-weight:780;letter-spacing:-.055em}.dashboard-layout{grid-column:1/-1;display:grid;grid-template-columns:minmax(0,8fr) minmax(220px,3fr);gap:var(--gap);align-items:start}.inbox{min-width:0;overflow:hidden}.marginalia{display:grid;gap:var(--gap);position:sticky;top:116px;min-width:0}.stat-card{padding:16px}.stat-list{display:grid;gap:0}.stat{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px;margin:0;padding:10px 0;border-top:1px solid var(--line);align-items:baseline;color:inherit;text-decoration:none}.stat:first-child{border-top:0}.stat span{color:var(--muted);font-size:13px;font-weight:600}.stat strong{font-family:var(--font-display);font-variant-numeric:tabular-nums;font-size:32px;font-weight:780;letter-spacing:-.055em}.unresolved-link span{display:grid;gap:2px}.unresolved-link em{font-style:normal;color:color-mix(in srgb,var(--accent) 55%,var(--muted));font-size:11px;font-weight:600}.unresolved-link:hover span{text-decoration:underline;text-underline-offset:3px}.empty.compact{margin:8px 0 0}.empty-state{display:grid;gap:4px;padding:22px;border:1px dashed var(--line);border-radius:var(--inner);background:color-mix(in srgb,var(--surface-2) 72%,transparent);color:var(--muted)}.empty-state strong{color:var(--ink);font-family:var(--font-display);font-size:24px;letter-spacing:-.04em}.empty-state p{margin:0}.empty-state.compact{padding:12px}.time-section{display:grid;gap:10px}.time-section h2{margin:4px 6px 0;color:var(--muted);font-family:var(--font-mono);font-size:11px;font-weight:600;letter-spacing:.14em;text-transform:uppercase}.item-list{display:grid;gap:10px}.pagination{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:12px;align-items:center;padding:12px 10px 14px;border-top:1px solid var(--line)}.pagination .muted{text-align:center;margin:0}.settings-form{display:grid;gap:14px;max-width:520px}.settings-form label{display:grid;gap:7px;font-weight:700}.settings-form .checkbox-row{grid-template-columns:auto minmax(0,1fr);align-items:center}.settings-form input[type=checkbox]{width:18px;height:18px;accent-color:var(--accent)}.settings-form select{min-height:42px;border:1px solid var(--line-strong);border-radius:8px;background:var(--surface-2);color:var(--ink);font:inherit;padding:0 12px}.item-list{display:grid;gap:10px}.item,.deploy-card,.setup-steps li{background:var(--surface-2);border:1px solid var(--line);border-radius:var(--inner);box-shadow:0 1px 0 rgba(255,255,255,.22) inset,0 1px 2px rgba(0,0,0,.06)}.inbox-list{padding:10px}.item{display:grid;grid-template-columns:96px minmax(0,1fr);gap:18px;align-items:start;padding:14px 16px;transition:background-color .42s cubic-bezier(.2,0,0,1),border-color .42s cubic-bezier(.2,0,0,1),box-shadow .42s cubic-bezier(.2,0,0,1)}.item-time{padding-top:2px;color:var(--muted);font-family:var(--font-mono);font-size:11px;font-weight:600;letter-spacing:-.03em;text-align:right;text-decoration:none;font-variant-numeric:tabular-nums}.item-time span,.item-time strong{display:block}.item-time strong{margin-top:3px;color:color-mix(in srgb,var(--ink) 70%,var(--muted));font-size:13px}.item-main{min-width:0}.item-topline{display:grid;grid-template-columns:auto minmax(0,1fr);gap:8px;align-items:start;min-width:0}.item-signals{display:grid;grid-auto-flow:column;grid-auto-columns:max-content;align-items:center;gap:4px;min-height:24px}.repo-avatar,.author-avatar{width:22px;height:22px;border-radius:999px;border:1px solid var(--line);background:var(--surface-3);display:block}.author-avatar{width:18px;height:18px;margin-left:-10px}.type-icon{display:grid;place-items:center;width:24px;height:24px;border-radius:999px;background:var(--surface-3);border:1px solid var(--line);color:color-mix(in srgb,var(--accent) 60%,var(--ink));font-family:var(--font-mono);font-size:13px;font-weight:600}.check-status{display:inline-grid;place-items:center;width:16px;height:16px;border-radius:999px;border:1px solid var(--line);background:var(--surface-3);align-self:center}.check-status:before{content:"";width:7px;height:7px;border-radius:999px;background:var(--muted)}.check-status.success:before{background:#4d9f5e}.check-status.failure:before{background:#d85b4a}.check-status.pending:before{background:#d79a35}.relative-time{color:var(--muted);font-weight:600}.item-title{display:block;margin:7px 0 5px;color:var(--ink);font-size:18px;font-weight:700;letter-spacing:-.025em;line-height:1.12;text-decoration:none}.item-title:hover{text-decoration:underline;text-decoration-thickness:2px;text-underline-offset:3px}.item p{margin-bottom:5px;color:var(--muted);line-height:1.35}.item .action{color:var(--ink);font-weight:700}.chips{display:flex;gap:6px;flex-wrap:wrap}.chip,.badge{display:inline-flex;align-items:center;min-height:24px;padding:0 9px;border-radius:6px;background:var(--surface-3);border:1px solid var(--line);color:color-mix(in srgb,var(--accent) 45%,var(--ink));font-family:var(--font-mono);font-size:11px;font-weight:600;letter-spacing:-.02em}
.refresh-summary{grid-column:1/-1;display:flex;align-items:baseline;gap:10px;padding:12px 14px;border:1px solid var(--line);border-radius:var(--inner);background:color-mix(in srgb,var(--accent) 12%,var(--surface-2));box-shadow:var(--button-shadow)}.refresh-summary strong{font-family:var(--font-display);font-size:22px;letter-spacing:-.04em}.refresh-summary span{color:var(--muted);font-weight:600}.refresh-summary.quiet{background:var(--surface-2)}.setup-status,.config-card{border:1px solid var(--line);border-radius:var(--inner);background:var(--surface-2);padding:12px;color:var(--ink)}.setup-status.ready{color:#315d38}html[data-theme=dark] .setup-status.ready{color:#add6b5}.setup-checks{display:grid;gap:8px;margin:12px 0}.setup-check{display:grid;grid-template-columns:28px minmax(0,1fr);gap:10px;padding:12px;border:1px solid var(--line);border-radius:var(--inner);background:var(--surface-2)}.setup-check p{margin:3px 0 0;color:var(--muted);line-height:1.35}.setup-check .fix{color:var(--ink);font-weight:600}.check-dot{display:grid;place-items:center;width:24px;height:24px;border-radius:999px;background:var(--surface-3);font-family:var(--font-mono);font-size:13px;font-weight:600}.setup-check.pass .check-dot{background:#d8ead4;color:#315d38}.setup-check.warn .check-dot{background:#f7dfad;color:#6b4b26}.setup-check.fail .check-dot{background:#f3c7bd;color:#7b2d21}html[data-theme=dark] .setup-check.pass .check-dot{background:#244633;color:#add6b5}html[data-theme=dark] .setup-check.warn .check-dot{background:#4c3b1f;color:#e1cfa8}html[data-theme=dark] .setup-check.fail .check-dot{background:#4a2525;color:#f0b2a7}.deploy-card,.config-card{display:grid;grid-template-columns:minmax(0,1fr) auto;justify-content:space-between;gap:16px;align-items:center;margin-top:12px;padding:14px}.deploy-card p{margin:4px 0 0;color:var(--muted)}.config-card{grid-template-columns:1fr;align-items:stretch}.config-card p{display:grid;grid-template-columns:minmax(140px,auto) minmax(0,1fr);gap:12px;margin:0;color:var(--muted)}.config-card code{display:block;min-width:0;overflow:auto;white-space:nowrap}.changelog-text{white-space:pre-wrap;overflow:auto;margin:16px 0 0;padding:16px;border:1px solid var(--line);border-radius:var(--inner);background:var(--surface-2);color:var(--ink);font-family:var(--font-mono);font-size:13px;line-height:1.5}.setup-steps{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;padding:0;margin:16px 0 0;list-style:none}.setup-steps li{display:grid;grid-template-columns:28px minmax(0,1fr);gap:12px;padding:14px}.setup-steps p{margin:4px 0 0;color:var(--muted);text-wrap:pretty}.step-number{height:28px;display:grid;place-items:center;border-radius:7px;background:var(--accent);color:var(--accent-ink);font-family:var(--font-mono);font-weight:600;font-variant-numeric:tabular-nums}code{font-family:var(--font-mono);font-size:.92em;padding:2px 5px;border:1px solid var(--line);border-radius:5px;background:color-mix(in srgb,var(--surface-2) 70%,white)}html[data-theme=dark] code{background:rgba(255,255,255,.075)}table{width:100%;min-width:640px;border-collapse:separate;border-spacing:0 8px}th{text-align:left;color:var(--muted);font-family:var(--font-mono);font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.08em}td,th{padding:10px 12px}td{background:var(--surface-2);border-block:1px solid var(--line)}td:first-child{border-left:1px solid var(--line);border-radius:8px 0 0 8px}td:last-child{border-right:1px solid var(--line);border-radius:0 8px 8px 0}
